...
- Auto-runs when a TXT book is opened and uses batched exact title matching by default
...
- Reads UTF-8 files directly and includes built-in GB18030/GBK compatibility
...
3. Open KOReader's default `Table of contents` entry to view the detected
   chapters.
4. If the TOC looks wrong or the TXT file changed, open `TXT 自动目录` in the
   reader menu and choose `立即生成/重建目录`.
...
- File reading validates UTF-8 first. If the file is not valid UTF-8, the plugin
  transcodes it from GB18030/GBK to UTF-8 before chapter detection.
...
- If batched exact matching cannot map enough headings, the plugin falls back to
  fast line/page estimation so the book still gets a usable TOC.
- Manual `立即生成/重建目录` also uses exact matching. On older KOReader builds
  without a progress dialog widget, it falls back to a plain notification.
...
- GB18030/GBK transcoding uses system `iconv` when available. If unavailable,
  the plugin falls back to its bundled decoder table.
